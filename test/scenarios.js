...
//                        assert.strictEqual( balances.vehicle1.toString(10), 0 );
    ...
    describe("scenario 6", function() {
		...
		it("should have two vehicles enter and exit with unknown route price and receive a refund", function() {

			return operator.enterRoad.call(booth1, hashed1, { from: vehicle1, value: 14 })
				.then(success => assert.isTrue(success))
				.then(() => operator.enterRoad(booth1, hashed1, { from: vehicle1, value: 14 }))
				.then(tx => {
					assert.strictEqual(tx.receipt.logs.length, 1);
					assert.strictEqual(tx.logs.length, 1);
					const logEntered = tx.logs[0];
				...
				.then(balance => {
					assert.strictEqual(balance.toNumber(), 14);
				...
				.then(info => {
					...
					assert.strictEqual(info.hashed1[2].toNumber(), 14);
					assert.strictEqual(info.pendingCount01.toNumber(), 1);
				})
				.then(result => {
